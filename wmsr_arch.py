...
class SpatialAttention(nn.Module):
    """
    空间注意力模块，关注输入特征图的空间维度，通过对每个通道的平均值和最大值进行卷积处理生成注意力图。
    """
    def __init__(self):
        super(SpatialAttention, self).__init__()
        # 使用反射填充的7x7卷积核，计算空间注意力图
        self.sa = nn.Conv2d(2, 1, 7, padding=3, padding_mode='reflect', bias=True)

    def forward(self, x):
        x_avg = torch.mean(x, dim=1, keepdim=True)  # 计算通道平均值
        x_max, _ = torch.max(x, dim=1, keepdim=True)  # 计算通道最大值
        x2 = torch.cat([x_avg, x_max], dim=1)  # 将平均值和最大值在通道维度上堆叠
        sattn = self.sa(x2)  # 计算空间注意力权重
        return sattn
...
class ChannelAttention(nn.Module):
    """
    通道注意力模块，关注输入特征图的通道维度，通过全局平均池化和两个1x1卷积层进行特征变换。
    参数:
    - dim: 输入特征的通道数
    - reduction: 降维比例
    """
    def __init__(self, dim, reduction=8):
        ...
        self.ca = nn.Sequential(
            nn.Conv2d(dim, dim // reduction, 1, padding=0, bias=True),  # 降维
            nn.ReLU(inplace=True),  # 激活函数
            nn.Conv2d(dim // reduction, dim, 1, padding=0, bias=True),  # 升维
        )

    def forward(self, x):
        x_gap = self.gap(x)  # 将空间维度缩减为1x1
        cattn = self.ca(x_gap)  # 通过1x1卷积调整通道数，生成通道注意力权重
        return cattn
...
class PixelAttention(nn.Module):
    ...
    def forward(self, x, pattn1):
        x = x.unsqueeze(dim=2)  # 增加一个维度
        pattn1 = pattn1.unsqueeze(dim=2)  # 增加一个维度
        x2 = torch.cat([x, pattn1], dim=2)  # 在新的维度上合并特征和第一阶段的注意力图
        x2 = x2.view(x2.size(0), -1, x2.size(3), x2.size(4))  # 重排维度以适配卷积操作
        pattn2 = self.pa2(x2)  # 计算第二阶段的注意力权重
        pattn2 = self.sigmoid(pattn2)  # 使用Sigmoid函数归一化注意力权重
        return pattn2
...
class CGAFusion(nn.Module):
    """
    内容引导注意力融合模块，结合空间注意力、通道注意力和像素级注意力，以此来增强特征融合效果。
    参数:
    - dim: 输入特征的通道数
    ...
    def __init__(self, dim, reduction=8):
        ...
        self.conv = nn.Conv2d(dim, dim, 1, bias=True)  # 用于最终的特征融合
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x, y):
        ...
        pattn1 = sattn + cattn  # 合并空间和通道注意力结果
        pattn2 = self.sigmoid(self.pa(initial, pattn1))  # 计算像素级注意力
        result = initial + pattn2 * x + (1 - pattn2) * y  # 根据注意力权重融合特征
        result = self.conv(result)  # 通过1x1卷积整合融合后的特征
        return result
...
class wiener_attention(nn.Module):

    def __init__(self, in_ch, out_ch):
        super(wiener_attention, self).__init__()
        self.conv_first = nn.Conv2d(in_ch, out_ch, kernel_size=3, stride=1, padding=1)
        self.conv_last = nn.Conv2d(out_ch, out_ch, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        _, _, w, h = x.shape
        self.complex_weight = nn.Parameter(torch.randn(1, w, h, 2, dtype=torch.float32)*0.02, requires_grad=True).cuda()
        self.learnable_K = nn.Parameter(torch.randn(1, 1, w, h, dtype=torch.float32), requires_grad=True).cuda()
        x = x.to(torch.float32)
        x_fft  = torch.fft.fft2(x, dim=(2, 3), norm='ortho')
        # x_fft = torch.fft.fftshift(x_fft).cuda()
        #kernel
        weight = torch.view_as_complex(self.complex_weight)
        weight = (torch.conj(weight) / ((torch.abs(weight) ** 2) + self.learnable_K))
        #filter
        x_fft = x_fft * weight
        # x = torch.fft.ifftshift(x_fft)
        
        x = torch.fft.ifft2(x_fft, s=(w, h), dim=(2, 3), norm='ortho')
        x = torch.abs(x)
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.wiener = wiener_attention(hidden_dim,hidden_dim)
    ...
    def forward(self, input, x_size):
        ...
        fft0 = self.wiener(input.permute(0, 3, 1, 2).contiguous())
        fft0 = fft0.permute(0, 2, 3, 1).contiguous()

        x = self.ln_1(input)  # B H W C
        x = input*self.skip_scale + self.drop_path(self.self_attention(x)) + fft0
        ...
        x = x*self.skip_scale2 + self.hybridgate(self.ln_2(x).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous() + fft  # B H W C
